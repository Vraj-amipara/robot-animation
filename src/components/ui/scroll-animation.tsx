  ...
  return (
    <ScrollAnimationContext.Provider value={{ scrollProgress }}>
      <div ref={scrollRef} className={cn('relative', className)} {...props}>
        {children}
        <div className={cn('w-full h-[150vh]', spacerClass)} />
      </div>
...
export function ScrollTranslateY({
  ...
  const y = useTransform(scrollProgress, inputRange as number[], yRange as number[]);
  return (
    <motion.div
...
export function ScrollTranslateX({
  ...
  const { scrollProgress } = useScrollAnimationContext();
  const x = useTransform(scrollProgress, inputRange as number[], xRange as (string | number)[]);
  return (
    <motion.div
...
export function ScrollScale({
  ...
  const { scrollProgress } = useScrollAnimationContext();
  const scale = useTransform(scrollProgress, inputRange as number[], scaleRange as number[]);
  return (
    <motion.div
      className={className}
