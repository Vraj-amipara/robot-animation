  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!serviceId || !templateId || !publicKey ||
        serviceId === 'service_t8td6ee' ||
        templateId === 'template_p73hgy2' ||
        publicKey === 'baHEoESiMEe2HqXSp') {
        console.warn('EmailJS credentials not properly configured. Please set VITE_EMAILJS_* environment variables.');
      ...
      const errorMessage = error?.text || error?.message || 'An error occurred while sending your message.';
    ...
    <div className="min-h-screen bg-base pt-24 pb-20 overflow-hidden">
      ...
      <section className="relative pt-32 pb-24 text-center mb-12 border-b border-light/5">
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-linear-to-b from-base via-base/80 to-base pointer-events-none"></div>
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,var(--color-accent),transparent_50%)]/10 pointer-events-none"></div>
        </div>
